...
- 低相关多因子 alpha 组合搜索：自动限制 `abs(corr) <= 0.30`
- 低相关 alpha 组合的严格交易过滤与 walk-forward 滚动验证
- 低相关 alpha 组合的风控搜索：专门搜索硬止损、ATR 追踪止损、持有期和总敞口，目标是把 OOS 最大回撤压到指定阈值以下
- 多策略研究、参数优化、冻结参数
...
7. [research/factor_combo_search.py](research/factor_combo_search.py)
8. [pipelines/run_factor_pipeline.py](pipelines/run_factor_pipeline.py)
9. [pipelines/run_alpha_combo_search.py](pipelines/run_alpha_combo_search.py)
10. [pipelines/run_alpha_combo_risk_search.py](pipelines/run_alpha_combo_risk_search.py)
11. [backtest/engine.py](backtest/engine.py)
12. [report/alpha-factor-sources.md](report/alpha-factor-sources.md)
...
如果你的首要目标是“先把样本外最大回撤压到 `15%` 以下”，最新风控搜索报告在：
...
- 优点是已经把样本外最大回撤压进了你希望的 `15%` 以内
- 代价是收益和 Sharpe 明显低于进攻版 alpha 组合
- 所以它更适合当保守基准，而不是收益最大化模板
...
- `factor_combo_search.max_pairwise_correlation`
- `alpha_trade_filters.max_true_range_pct_1`
...
如果你想专门做“多因子组成一个大 alpha”的实验，而不是整条主流水线，可以直接跑：

```bash
conda activate quant
python main.py alpha-combo-search --train-start 2016-01-01 --train-end 2021-12-31 --oos-start 2022-01-01 --oos-end 2024-12-31
```

这个命令会：

- 先用单因子结果确认每个因子的有效方向
- 允许把稳定负向因子翻成正向 alpha 后再组合
- 只保留两两 `abs(corr) <= 0.30` 的组合
- 自动尝试多组 2 到 4 因子组合
- 分别输出 train / OOS 组合回测结果
- 冻结当前最好的低相关组合模型

如果你想继续看这套低相关组合在滚动窗口里是不是稳定，可以再跑：

```bash
conda activate quant
python main.py alpha-combo-walk-forward --start 2016-01-01 --end 2024-12-31 --train-years 3 --test-months 12 --step-months 12
```

这个命令会：

- 每个训练窗口里重新选一组低相关 alpha 组合
- 组合选择只看训练窗，不偷看下一段测试窗
- 把严格交易过滤也一起带进 OOS
- 输出每个窗口选中了什么因子、什么权重、以及测试窗表现
