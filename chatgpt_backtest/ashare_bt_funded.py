...
# Corrected interpretation: waiting cash is part of total strategy principal when
# computing cost-basis target weights. This prevents target weights from becoming
# permanently "full" while new monthly contributions sit outside the denominator.
def score_funded(t,s,d,tw,cb,principal_base):
    x=score(t,s,d,tw,cb)
    oldg=x['G']
...
def one_lot_cost(t,d):
    ...
    n=lot(t); nom=n*rp; return nom+max(5,.0003*nom)+.00001*nom+.0005*nom

def run_funded(real=False,filldown=False):
    ...
    for d in CAL:
        if d==months[(d.year,d.month)]:
            ...
            if m==1:
                ...
                # Existing waiting cash stays waiting, but it counts in principal base.
                ...
                if real:
                    ...
                    while rem>0:
                        ...
                        for t,w in tw.items():
                            ...
                            if st['cb'].get(t,0)+c>target_amt[t]+1e-9: continue
                        ...
                        z=buy(st,t,d,rem,True,'annual-funded')
                        if not z: break
                        rem-=z; spent+=z
                    st['cash']+=rem
                else:
                    ...
                    if z:
                        for t,g in gaps.items():
                            a=min(budget*g/z,g)
                            if a>1e-9: spent+=buy(st,t,d,a,False,'annual-funded')
                    st['cash']+=budget-spent
                st['actions'].append({'month':d.strftime('%Y-%m'),'type':'annual','invested':spent,'cash':st['cash']})
            else:
                C=sum(st['cb'].values()); base=C+st['cash']+fresh
                ss=[]
                for t,w in tw.items():
                    sf=score_funded(t,sect(t,L),d,w,st['cb'],base)
                    st['scores'].append({'date':str(d.date()),**sf})
                    if (not sf['veto']) and st['cb'].get(t,0)<w*base-1e-9: ss.append(sf)
                ...
                if not ss or ss[0]['total']<70:
                    ...
                    if real:
                        if filldown:
                            rem=dep
                            order=primary+[x for x in ss if x not in primary and x['total']>=70]
                            for x in order:
                                t=x['ticker']
                                while rem>0:
                                    c=one_lot_cost(t,d)
                                    if not c or c>rem or st['cb'].get(t,0)+c>target_amt[t]+1e-9: break
                                    z=buy(st,t,d,rem,True,'score-funded-fill')
                                    if not z: break
                                    rem-=z; spent+=z
                        else:
                            ...
                            for x in primary:
                                ...
                                while rem>0:
                                    c=one_lot_cost(t,d)
                                    if not c or c>rem or st['cb'].get(t,0)+c>target_amt[t]+1e-9: break
                                    z=buy(st,t,d,rem,True,'score-funded-top')
                                    if not z: break
                                    rem-=z; spent+=z
                    else:
                        zsum=sum(x['total'] for x in primary); rem_extra=0.0
                        for x in primary:
                            t=x['ticker']; a0=dep*x['total']/zsum
                            gap=max(target_amt[t]-st['cb'].get(t,0),0); a=min(a0,gap)
                            if a>1e-9: spent+=buy(st,t,d,a,False,'score-funded-top')
                            rem_extra+=a0-a
                        if filldown and rem_extra>1e-9:
                            order=[x for x in ss if x not in primary and x['total']>=70]
                            for x in order:
                                if rem_extra<=1e-9: break
                                t=x['ticker']; gap=max(target_amt[t]-st['cb'].get(t,0),0); a=min(rem_extra,gap)
                                if a>1e-9:
                                    spent+=buy(st,t,d,a,False,'score-funded-fill'); rem_extra-=a
...
# Lixinger monthly PRICE-index returns, Jan-2021 ... Dec-2025.
...
def bench_monthly(rets):
    ...
    for (d0,d1),r in zip(months,rets):
        cny=AUD*FX.loc[d0]; units+=cny/p; flows.append((d0,-AUD)); p*=1+r/100
        vals.append(units*p/FX.loc[d1])
    ...
    s=pd.Series(vals,index=[x[1] for x in months])
    idx=np.cumprod(1+np.array(rets)/100); idxdd=float((idx/np.maximum.accumulate(idx)-1).min())
    return {'terminal_aud':ta,'profit_aud':ta-AUD*60,'xirr':float((lo+hi)/2),
            'account_monthly_max_dd':float((s/s.cummax()-1).min()),'index_monthly_max_dd':idxdd}

FTS,FTR=run_funded(False,False)
BTS,BTR=run_funded(True,False)
FFS,FFR=run_funded(False,True)
BFS,BFR=run_funded(True,True)
SUMMARY={'meta':{'period':'2021-2025','contrib_aud':18000,'fx':FXSRC,'interpretation':'waiting cash included in cost-basis target-weight denominator','filldown':'if primary Top1/Top2 cannot absorb allowed deployment, roll leftover to next score>=70'},
         'old_fractional':FRES,'old_board_lot':RRES,
         'funded_top_fractional':FTR,'funded_top_board_lot':BTR,
         'funded_fill_fractional':FFR,'funded_fill_board_lot':BFR,
...
with open('chatgpt_backtest/output/funded_summary.json','w',encoding='utf-8') as f:json.dump(SUMMARY,f,ensure_ascii=False,indent=2,default=str)
for n,st in [('funded_top_fractional',FTS),('funded_top_board_lot',BTS),('funded_fill_fractional',FFS),('funded_fill_board_lot',BFS)]:
    pd.DataFrame(st['actions']).to_csv(f'chatgpt_backtest/output/{n}_actions.csv',index=False)
print('FUNDED_SUMMARY_BEGIN');print(json.dumps(SUMMARY,ensure_ascii=False,indent=2,default=str));print('FUNDED_SUMMARY_END')
